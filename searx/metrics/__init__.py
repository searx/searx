...
def initialize(engine_names=None):
    ...
    for endpoint in ENDPOINTS:
        histogram_storage.configure(histogram_width, histogram_size, 'endpoint', endpoint)

    # search class
    histogram_storage.configure(histogram_width, histogram_size, 'search', 'time')
...
def get_engine_errors(engline_list):
    ...
    for engine_name in engine_names:
        if engine_name is not engline_list:
            continue
...
def get_engines_stats(engine_list):
    ...
    for engine_name in engine_list:
        ...
        time_total = histogram('engine', engine_name, 'time', 'total').average
        time_http = histogram('engine', engine_name, 'time', 'http').average
