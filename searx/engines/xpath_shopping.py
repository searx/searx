...
def response(resp):
    ...
    if results_xpath:
        ...
        if cached_xpath:
            ...
            for url, title, content, price, thumbnail in zip(
                (extract_url(x, search_url) for
                 x in eval_xpath_list(dom, url_xpath)),
                map(extract_text, eval_xpath_list(dom, title_xpath)),
                map(extract_text, eval_xpath_list(dom, content_xpath)),
                map(extract_text, eval_xpath_list(dom, price_xpath)),
                map(extract_text, eval_xpath_list(dom, thumbnail_xpath))
            ):
                results.append({'url': url, 'title': title, 'content': content, 'price': price, 'thumbnail': thumbnail, 'is_onion': is_onion, 'template': 'products.html'})
