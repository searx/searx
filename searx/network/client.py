...
def new_client(enable_http, verify, http2,
    ...
    for pattern, proxy_url in iter_proxies(proxies):
        ...
        if proxy_url.startswith('socks4://') \
           ...
           or proxy_url.startswith('socks5h://'):
            mounts[pattern] = get_transport_for_socks_proxy(verify, http2, local_address, proxy_url, limit, retries)
        else:
            mounts[pattern] = get_transport(verify, http2, local_address, proxy_url, limit, retries)

    if not enable_http:
        mounts['http://'] = AsyncHTTPTransportNoHttp()

    transport = get_transport(verify, http2, local_address, None, limit, retries)
